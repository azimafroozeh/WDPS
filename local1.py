...
#rddc = rdd2.collect()
#rdd3 = sc.parallelize(rddc, 100).flatMap(tokenizer).saveAsTextFile(OUTFILE)
#rdd3 = sc.parallelize(rddc, 100).flatMap(tokenizer)
rdd = sc.textFile("hdfs:///user/wdps1810/NERresult")
tuple_rdd = rdd.flatMap(lambda x: [eval(x)])
ftuple=tuple_rdd.flatMap(lambda x:file2tuple(x))
result=ftuple.flatMap(lambda x:search(EHost,x)).saveAsTextFile(OUTFILE)
